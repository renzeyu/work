...
import styles from "./ShapePlaygroundPreview.module.css";

type PlaygroundEngine = ReturnType<
  typeof import("./shapePlaygroundEngine").createShapePlaygroundEngine
>;

type ShapePlaygroundPreviewProps = {
  instructionsId: string;
};

export function ShapePlaygroundPreview({
  instructionsId,
}: ShapePlaygroundPreviewProps) {
  ...
  useEffect(() => {
    autoSpawnControllerRef.current?.setEligible(canSpawn && runtimeEligible);
  ...
  return (
    <div className={styles.preview} data-shape-playground="true">
      <button
        ref={frameRef}
